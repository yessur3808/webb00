...
var config = {
	...
	workExp: [

		{
			...
			desc: "Description",
			points: [
				" My work consists is on both frontend & backend services",
				" I have worked on <a href='https://www.gryfyn.io/' target='_blank' rel='noreferrer noopener' >Gryfyn</a> Frontend for both the wallet and the main website",
				" I have worked on <a href='https://hextrust.com/products/hex-safe' target='_blank' rel='noreferrer noopener' >Hexsafe</a> Frontend",
				" Web SDK Development",
				" Blockchain integrated Services",
				" Several backend services for both Gryfyn & Hexsafe products"
			],
		...
		},
		{
			...
			imgSrc: "img/logos/cryto_com.svg",
...

		{
			...
			imgSrc: "img/logos/tradefi_white.png",
			...
			imgSrc: "img/logos/scmpOffice.jpg",
			...
			imgSrc: "img/logos/sunnydayedu.png",
...
function inputPositions() {
	...
	for (var i = 0; i < config.workExp.length; i++) {
		...
		if (job.type === 'work') {
			...
			jobExp += '<div id="panel-' + i + '" role="tabpanel" tabindex="' + i + '" aria-labelledby="tab-' + i + '" aria-hidden="false" class="jobsTabContent"><div class="job-logo"><img src="' + job.imgSrc + '" alt="' + job.company + '" loading="lazy"></div><h3><span>' + job.position + '</span><span class="company"><a href="' + job.website + '" class="inline-link" target="_blank" rel="noreferrer noopener">&nbsp;@&nbsp;' + job.company + '</a></span></h3><p class="range">' + date0 + ' - ' + date1 + '</p><div>' + tempPoints + '</div><div class="skills">' + skillsTemp + '</div></div>';
		}
...
function countersInit() {
	if (config.isInView(document.querySelector('footer .counters .counter')) && !config.counterbool) {
		...
		setTimeout(function () {
			animateVal('footer .counters .counter.coffee span', parseInt(document.querySelector('footer .counters .counter.coffee').innerText), 7432, 1600);
		}, 600);
